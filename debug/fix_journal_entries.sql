-- Fix the journal entry function to handle UUID type correctly
DROP FUNCTION IF EXISTS public.fix_existing_allocation_journal_entries();
...
BEGIN
    ...
    LOOP
        BEGIN
            -- Create journal entry with proper UUID casting
            INSERT INTO public.journal_entries (
                ...
                ' advance to ' || v_allocation.officer_id::text ||
                ' for period ' || v_allocation.allocated_period || ' (retroactive fix)',
                COALESCE(v_allocation.created_at::DATE, CURRENT_DATE),
                'allocation',
                v_allocation.id::text,  -- This will be implicitly cast to UUID
                'posted'
            )
