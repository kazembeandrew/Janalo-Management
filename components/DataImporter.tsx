...
export const DataImporter: React.FC = () => {
  ...
  const { isLoanOfficer } = usePermissions();
    ...
    try {
      ...
      for (const row of preview.rows) {
        ...
        if (preview.type === 'loans') {
          ...
          // FIX: DataImporter currently hardcodes to 'flat' rate, so interest_outstanding = interest is correct
          // If support for reducing balance imports is added later, apply same logic as createLoan
          const importedInterestType = 'flat'; // Currently hardcoded
          const importedInterestOutstanding = importedInterestType === 'flat' ? interest : 0;
          ...
          const { data: loanRow, error: loanError } = await supabase
            ...
            .insert([{
              ...
              interest_type: importedInterestType,
              status: 'pending',
              principal_outstanding: principal,
              interest_outstanding: importedInterestOutstanding,
              total_payable: total,
