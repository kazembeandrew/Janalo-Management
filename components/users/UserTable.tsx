...
import { UserCog, Copy, RefreshCw, Smartphone } from 'lucide-react';
          ...
          users.map(user => (
            <div 
              ...
              className={`border-b border-gray-100 last:border-b-0 p-4 hover:bg-gray-50 transition-colors ${
                user.deletion_status === 'approved' ? 'opacity-60 grayscale-[0.5]' : ''
              }`}
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center">
                  <div className={`h-12 w-12 rounded-xl flex items-center justify-center font-bold text-sm ${
                    user.deletion_status === 'approved' ? 'bg-gray-200 text-gray-500' : 'bg-indigo-100 text-indigo-700'
                  }`}>
                  ...
                  <div className="ml-3">
                    <div className="text-sm font-bold text-gray-900 flex items-center">
                      ...
                      {user.deletion_status === 'approved' && (
                        <span className="ml-2 px-2 py-0.5 bg-gray-100 text-gray-500 text-[10px] font-bold uppercase rounded border border-gray-200">Archived</span>
                      )}
                      {user.deletion_status === 'pending_approval' && (
                        <span className="ml-2 px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] font-bold uppercase rounded border border-amber-200">Pending CEO Approval</span>
                      )}
                    </div>
                    <div className="text-xs text-gray-500 flex items-center">
                      {user.email}
                      <button onClick={() => onCopy(user.email)} className="ml-2 opacity-0 group-hover:opacity-100 transition-opacity text-gray-400 hover:text-indigo-600">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  ...
                  <div className="mt-1">
                    <span className="px-2 py-1 rounded-lg bg-gray-100 text-gray-700 uppercase w-fit border border-gray-200 text-[10px] font-bold">
                      {user.role.replace('_', ' ')}
                    </span>
                    {user.delegated_role && (
                      <div className="mt-1 flex items-center text-[10px] text-indigo-600 font-bold bg-indigo-50 px-2 py-0.5 rounded-md w-fit border border-indigo-100">
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">User Details</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Primary Role</th>
