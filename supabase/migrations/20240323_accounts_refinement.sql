-- 1. Add balance column to internal_accounts if not exists
ALTER TABLE public.internal_accounts
ADD COLUMN IF NOT EXISTS balance DECIMAL(15,2) DEFAULT 0;

-- 2. Code is already unique in base schema (skipping)

-- 3. Enforce valid accounting categories for the double-entry engine
-- UNIQUE CONSTRAINT ALREADY EXISTS (skipping)

-- 4. Add a flag to identify mandatory system accounts
ALTER TABLE public.internal_accounts
ADD COLUMN IF NOT EXISTS is_system_account BOOLEAN DEFAULT false;

-- 5. Mark existing core accounts as system accounts
UPDATE public.internal_accounts
SET is_system_account = true
WHERE code IN ('CAPITAL', 'EQUITY', 'BANK', 'CASH');

-- 6. Update the balance calculation trigger to be more robust
...
DECLARE
  v_category text;
BEGIN
  SELECT category INTO v_category FROM public.internal_accounts WHERE id = NEW.account_id;
