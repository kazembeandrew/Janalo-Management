...
export const notifyExecutivesForPendingUserArchive = async (params: {
  ...
  return createDedupeAwareNotifications({
    ...
    title: 'Executive Authorization Required: User Erasure',
    message: `A request to erase/archive user ${params.fullName} is pending authorization.`,
    category: 'security',
    priority: 'high',
    link: '/oversight',
    excludeUserId: params.excludeUserId,
    senderId: params.senderId,
    actions: [
      { label: 'Authorize Erase', action: 'authorize_erase', url: '/oversight', primary: true },
      { label: 'View Users', action: 'view_users', url: '/users', primary: false }
    ]
  });
};
