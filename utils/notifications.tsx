...
export const notification = {
  ...
  /**
   * Show error notification with optional error details
   */
  error: (message: string, error?: any, options?: ToastOptions) => {
    const errorMessage = error?.details || error?.message || message;
  ...
  /**
   * Show notification with promise
   */
  promise: async <T,>(
    promise: Promise<T>,
      ...
      promise,
      {
        ...
        error: (err) => {
          const errorMsg = typeof messages.error === 'function' 
            ? messages.error(err) 
            : messages.error;
          return notification.error(errorMsg, err);
        },
      },
      {
        loading: { ...defaultOptions, duration: Infinity },
        success: { ...defaultOptions },
        error: { ...defaultOptions, duration: 6000 },
      }
    );
  },
...
export const createNotificationFromTemplate = async (
  ...
  const { data, error } = await (supabase as any).rpc('create_notification_from_template', {
...
export const batchCreateNotifications = async (
  ...
  const { data, error } = await (supabase as any).rpc('batch_create_notifications', {
    p_user_ids: userIds,
    p_title: title,
    p_message: message,
...
export const markNotificationAsRead = async (notificationId: string): Promise<boolean> => {
  const { data, error } = await (supabase as any).rpc('mark_notification_read', {
...
export const archiveNotification = async (notificationId: string): Promise<boolean> => {
  const { data, error } = await (supabase as any).rpc('archive_notification', {
    p_notification_id: notificationId,
    p_user_id: (await supabase.auth.getUser()).data.user?.id
  });
