...
export const generateReceiptPDF = (loan: any, repayment: any, officerName: string) => {
    ...
    doc.autoTable({
        ...
        body: [
            ['Principal Paid', (Number(repayment.principal_paid) || 0).toFixed(2)],
            ['Interest Paid', (Number(repayment.interest_paid) || 0).toFixed(2)],
            ['Penalty Paid', (Number(repayment.penalty_paid) || 0).toFixed(2)],
            [{ content: 'TOTAL RECEIVED', styles: { fontStyle: 'bold' } }, { content: (Number(repayment.amount_paid) || 0).toFixed(2), styles: { fontStyle: 'bold' } }]
        ],
        theme: 'striped',
