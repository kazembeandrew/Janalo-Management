...
import { Plus, Search, Filter, Receipt, Calendar, Trash2, PieChart as PieIcon, TrendingUp, BarChart3, RefreshCw, X, AlertCircle } from 'lucide-react';
import { 
...
export const Expenses: React.FC = () => {
  ...
  const monthlyData = Array.from({ length: 6 }, (_, i) => {
      ...
      return {
          ...
          amount: expenses.filter(e => e.date.startsWith(monthStr)).reduce((sum, e) => sum + Number(e.amount), 0),
          rawMonth: monthStr
      };
  }).reverse();
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between">
              ...
              <div className="mt-6 pt-6 border-t border-gray-50">
                  <div className="flex justify-between text-sm mb-2">
                      ...
                      <span className="font-bold text-gray-900">{categoryData[0]?.name || 'N/A'}</span>
                  </div>
          ...
          <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                      {loading ? (
                          <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-500"><RefreshCw className="h-6 w-6 animate-spin mx-auto mb-2" /> Loading records...</td></tr>
                              ...
                              <tr key={expense.id} className="hover:bg-gray-50 transition-colors group">
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                      {isAccountant ? (
                                          ...
                                          <span className="text-gray-300 cursor-not-allowed" title="View Only"><AlertCircle className="h-4 w-4 inline" /></span>
                                      )}
                                  </td>
                              </tr>
                          ))
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                  <div className="bg-indigo-900 px-6 py-5 flex justify-between items-center">
                      <h3 className="font-bold text-white flex items-center text-lg"><Receipt className="mr-3 h-6 w-6 text-indigo-300" /> Record Expense</h3>
                  ...
                  <form onSubmit={handleSubmit} className="p-8 space-y-5">
                      ...
                      <div className="pt-4">
                          ...
                          >
                              {isProcessing ? 'Saving...' : 'Save Expense Record'}
                          </button>
                      </div>
                  </form>
              </div>
