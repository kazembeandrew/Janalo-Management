...
import { Shield, User, Power, Lock, Search, Plus, X, Mail, Key, Save, AlertTriangle, Trash2, Check, ArrowRight, UserPlus, Briefcase, Landmark, RefreshCw, Calendar, TrendingUp } from 'lucide-react';
...
export const Users: React.FC = () => {
  ...
  const handlePromote = async () => {
      ...
      // If promoting AWAY from Loan Officer, check for portfolio
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
      ...
      try {
          ...
          toast.success("User registered. Awaiting CEO approval.");
  ...
  return (
    <div className="space-y-6">
                          ...
                          <div className="space-y-4">
                              <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-100 flex items-start">
                                  ...
                                  <p className="text-xs text-yellow-700">
                                      <strong>Promotion is permanent.</strong> This will replace the user's current role. If promoting a Loan Officer, their portfolio must be reassigned.
                                  </p>
                              </div>
                              <div>
      ...
      {/* REASSIGNMENT MODAL (FOR PROMOTION) */}
      {showReassignModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black bg-opacity-75">
              <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                  <h3 className="text-lg font-bold text-red-600 flex items-center mb-4">
                      <RefreshCw className="h-5 w-5 mr-2 animate-spin-slow" /> Mandatory Reassignment
                  </h3>
                  <p className="text-sm text-gray-600 mb-6">
                      <strong>{selectedUser?.full_name}</strong> is currently a Loan Officer with active records. You must transfer their portfolio before promoting them to a non-officer role.
                  </p>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Select Successor Officer</label>
                  <select 
                  ...
                  <div className="flex gap-3">
                      <button onClick={() => setShowReassignModal(false)} className="flex-1 px-4 py-2 border rounded-md text-sm font-medium">Cancel</button>
                      <button 
                        onClick={handlePromote}
                        disabled={!successorId || isProcessing}
                        className="flex-1 bg-red-600 text-white px-4 py-2 rounded-md text-sm font-bold"
                      >
                          Transfer & Promote
                      </button>
                  </div>
