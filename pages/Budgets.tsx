...
export const Budgets: React.FC = () => {
  ...
  const cacheRef = useRef<Map<string, { budgets: Budget[], actuals: any[], timestamp: number }>>(new Map());
  ...
  const fetchData = async () => {
    ...
    try {
        ...
        const [budgetsPromise, expensesPromise, repaymentsPromise] = await Promise.all([
            // 1. Fetch Budgets
            (supabase as any)
                .from('budgets')
                .select('*')
        ...
        const { data: bData } = budgetsPromise;
        const { data: eData } = expensesPromise;
        const { data: rData } = repaymentsPromise;

        const budgetsData = bData || [];
  ...
  const handleSaveBudget = async (e: React.FormEvent) => {
      ...
      try {
          ...
          // Clear cache for this month to force fresh data fetch
          cacheRef.current.delete(month);
          fetchData();
      } catch (e: any) {
