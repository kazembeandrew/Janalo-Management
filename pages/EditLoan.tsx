...
export const EditLoan: React.FC = () => {
  ...
  const { canEditLoan } = usePermissions();
  ...
  const fetchInitialData = async () => {
      ...
      try {
          ...
          const { data: loan, error } = await (supabase as any).from('loans').select('*').eq('id', id).single();
          ...
          // 2. Fetch Borrowers
          let bQuery = (supabase as any).from('borrowers').select('*');
          if (hasRole('loan_officer')) {
          ...
          const { data: docData } = await (supabase as any).from('loan_documents').select('*').eq('loan_id', id);
          ...
          if (docData) {
              ...
              for (const doc of docData) {
                  const { data: publicUrlData } = (supabase as any).storage.from('loan-documents').getPublicUrl(doc.storage_path);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await (supabase as any).from('loans').update(loanUpdate).eq('id', id);
      if (error) throw error;
      ...
      const handleDocUpdate = (blob: Blob | null, type: string, namePrefix: string, friendlyName: string) => {
          ...
          uploads.push(async () => {
               await (supabase as any).from('loan_documents').delete().eq('loan_id', id).eq('type', type);
               const uploadedPath = await uploadFile(blob, path);
               await (supabase as any).from('loan_documents').insert({
                    loan_id: id,
                    type: type,
