...
export const Messages: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedConvo) {
      ...
      const presenceChannel = (supabase as any).channel(`typing-${selectedConvo.conversation_id}`, {
      ...
      return () => {
        (supabase as any).removeChannel(presenceChannel);
        (supabase as any).removeChannel(msgChannel);
      };
    }
  }, [selectedConvo, profile]);

  const fetchConversations = async () => {
    const { data, error } = await (supabase as any).rpc('get_my_conversations_details');
  ...
  const fetchBorrowers = async () => {
      ...
      if (!error) setBorrowers((data as any[]) || []);
  };
  ...
  const handleTyping = () => {
      ...
      const channel = (supabase as any).channel(`typing-${selectedConvo.conversation_id}`);
      channel.track({
          is_typing: true,
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
          ...
          const { error: msgError } = await (supabase as any).from('direct_messages').insert({
  ...
  const startNewChat = async (recipient: UserListItem) => {
      ...
      try {
          ...
          const { data: convoId, error } = await (supabase as any).rpc('create_new_conversation', {
  ...
  const renderMessageContent = (msg: Message, isMe: boolean) => {
      ...
      if (msg.attachment_path) {
          ...
          const { data: pUrl } = (supabase as any).storage.from('loan-documents').getPublicUrl(msg.attachment_path);
          const url = (pUrl as any).publicUrl;
