...
export const BorrowerDetails: React.FC = () => {
  ...
  const fetchBorrowerData = async () => {
    if (!id) return;
    try {
      const { data: bData, error: bError } = await (supabase as any).from('borrowers').select('*').eq('id', id).single();
      if (bError) throw bError;
      setBorrower(bData);

      const { data: lData } = await (supabase as any).from('loans').select('*').eq('borrower_id', id).order('created_at', { ascending: false });
      setLoans(lData || []);

      if (lData && lData.length > 0) {
          const loanIds = lData.map(l => l.id);
          const { data: dData } = await (supabase as any).from('loan_documents').select('*').in('loan_id', loanIds).order('created_at', { ascending: false });
          if (dData) {
              setAllDocuments(dData);
              const urlMap: {[key: string]: string} = {};
              for (const doc of dData) {
                  const { data: pUrl } = (supabase as any).storage.from('loan-documents').getPublicUrl(doc.storage_path);
  ...
  return (
    <div className="space-y-6">
      <button onClick={() => navigate('/borrowers')} className="inline-flex items-center text-xs font-bold uppercase tracking-wider text-gray-500 hover:text-indigo-600 transition-colors p-2 -ml-2 rounded-lg hover:bg-indigo-50">
        <ArrowLeft className="h-4 w-4 mr-2" /> Back to Borrowers
      </button>

      <div className="bg-white shadow-xl rounded-3xl overflow-hidden border border-gray-100">
        <div className="bg-[#0d1929] px-8 py-12 text-white relative overflow-hidden">
            <div className="absolute top-0 right-0 p-8 opacity-5">
                <User className="h-64 w-64" />
            </div>
            <div className="absolute inset-0 bg-gradient-to-br from-indigo-900/50 via-transparent to-transparent opacity-30"></div>
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div className="flex items-center">
                    <div className="h-28 w-28 rounded-3xl bg-indigo-500/20 backdrop-blur-xl border border-white/10 flex items-center justify-center text-5xl font-bold shadow-2xl shadow-black/20">
                        {borrower.full_name.charAt(0)}
                    </div>
                    <div className="ml-8">
                        <h1 className="text-4xl font-extrabold tracking-tight">{borrower.full_name}</h1>
                        <div className="flex flex-wrap gap-3 mt-4">
                           <span className="inline-flex items-center px-3 py-1 bg-white/5 backdrop-blur-md rounded-lg text-[10px] font-bold uppercase tracking-widest text-indigo-300 border border-white/5">
                               <Clock className="h-3 w-3 mr-2" /> Since {new Date(borrower.created_at).toLocaleDateString()}
                           </span>
                           <span className="inline-flex items-center px-3 py-1 bg-white/5 backdrop-blur-md rounded-lg text-[10px] font-bold uppercase tracking-widest text-emerald-300 border border-emerald-500/10">
                               <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-2 animate-pulse" /> Verified Profile
                           </span>
                        </div>
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-10">
                    <div>
                        <p className="text-[10px] uppercase tracking-widest text-indigo-300 font-bold mb-2">Total Borrowed</p>
                        <p className="text-3xl font-extrabold">{formatCurrency(totalBorrowed)}</p>
                    </div>
                    <div>
                        <p className="text-[10px] uppercase tracking-widest text-indigo-300 font-bold mb-2">Current Balance</p>
                        <p className="text-3xl font-extrabold text-orange-400">{formatCurrency(totalOutstanding)}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="space-y-8">
                <div>
                    <h3 className="text-[11px] font-bold text-gray-500 uppercase tracking-[0.2em] border-b border-gray-100 pb-4 mb-8">Contact & Location</h3>
                ...
                <div className="bg-gray-50/50 rounded-2xl p-6 border border-gray-100 shadow-inner group-hover:border-indigo-100 transition-all">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-[11px] font-bold text-indigo-900 uppercase tracking-widest flex items-center">
                        ...
                        {!riskAssessment && (
                            <button 
                                ...
                                className="text-[10px] bg-[#0d1929] text-white px-4 py-2 rounded-xl font-bold hover:bg-black disabled:bg-gray-400 transition-all shadow-md active:scale-95"
                            >
                    ...
                    {riskAssessment ? (
                        <div className="prose prose-sm prose-indigo max-w-none text-[12px] text-gray-700 leading-relaxed">
                            <Markdown>{riskAssessment}</Markdown>
                            <button onClick={() => setRiskAssessment(null)} className="mt-6 text-[10px] font-bold text-red-500 hover:text-red-600 uppercase tracking-wider flex items-center">
                                <X className="h-3 w-3 mr-1" /> Discard Analysis
                            </button>
                        </div>
                    ) : (
                        <p className="text-[11px] text-gray-500/80 italic leading-relaxed">Generate an automated risk profile based on this client's historical data, location stability, and repayment patterns.</p>
                    )}
                </div>
            </div>

            <div className="lg:col-span-2" id="borrower-tabs">
                <div className="flex border-b border-gray-100 mb-8 bg-gray-50/50 p-1 rounded-xl w-fit">
                    <button onClick={() => setActiveTab('loans')} className={`px-6 py-2.5 text-[10px] font-bold uppercase tracking-[0.1em] rounded-lg transition-all ${activeTab === 'loans' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600 hover:bg-white/50'}`}>Loan History</button>
                    <button onClick={() => setActiveTab('documents')} className={`px-6 py-2.5 text-[10px] font-bold uppercase tracking-[0.1em] rounded-lg transition-all ${activeTab === 'documents' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600 hover:bg-white/50'}`}>Documents</button>
                    <button onClick={() => setActiveTab('map')} className={`px-6 py-2.5 text-[10px] font-bold uppercase tracking-[0.1em] rounded-lg transition-all flex items-center ${activeTab === 'map' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600 hover:bg-white/50'}`}>
                        <MapIcon className="h-3.5 w-3.5 mr-2 shadow-sm" />
                        Location Map
                    </button>
                </div>
