...
export const CreateLoan: React.FC = () => {
  ...
  const { canCreateLoan, hasRole } = usePermissions();
  ...
  useEffect(() => {
    const fetchBorrowers = async () => {
        ...
        let query = (supabase as any).from('borrowers').select('*, loans(id, status, officer_id)');
  ...
  const handleSubmit = async () => {
    ...
    const { data: loan, error } = await (supabase as any).from('loans').insert([loanData]).select().single();
      ...
      const addUpload = (blob: Blob | null, type: string, namePrefix: string, friendlyName: string) => {
        ...
        uploads.push(
          uploadFile(blob, path)
            .then(uploadedPath => (supabase as any).from('loan_documents').insert({
              loan_id: loan.id,
              type: type,
