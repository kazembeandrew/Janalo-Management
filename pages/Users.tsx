...
export const Users: React.FC = () => {
  ...
  const { canArchiveUser, hasRole, hasAnyRole } = usePermissions();
  ...
  const isCEO = hasAnyRole(['ceo', 'admin']);
  ...
  const handleRevokeAccess = async () => {
      ...
      try {
          if (successorId) {
              const { error } = await (supabase as any).rpc('reassign_officer_portfolio', {
  ...
  const handlePermanentDelete = async () => {
      ...
      setIsProcessing(true);
      try {
          const { error } = await (supabase as any).from('users').delete().eq('id', selectedUser.id);
  ...
  const filteredUsers = users.filter(u => {
    ...
    const potentialSuccessors = users.filter(u => hasRole('loan_officer') && u.is_active && u.id !== selectedUser?.id);
                      ...
                      <div className="space-y-6">
                          ...
                          <div className="flex gap-3 pt-4">
                              ...
                              <button 
                                ...
                                disabled={isProcessing || !revocationReason || (hasRole('loan_officer') && !successorId)}
