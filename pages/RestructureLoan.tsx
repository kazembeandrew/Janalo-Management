...
export const RestructureLoan: React.FC = () => {
  ...
  const handleRestructure = async (e: React.FormEvent) => {
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          // FIX: Initialize interest_outstanding based on loan type (same logic as createLoan)
          const restructureInterestOutstanding = formData.interest_type === 'flat' 
            ? preview.totalInterest 
            : 0;

          const { data: newLoan, error: newError } = await (supabase as any)
            .from('loans')
            .insert([{
                ...
                interest_outstanding: restructureInterestOutstanding,
                status: 'pending'
            }])
            .select()
            .single();
          ...
          const { error: oldError } = await (supabase as any)
            .from('loans')
            .update({ 
                status: 'completed',
          ...
          // 3. Add Audit Notes
          await Promise.all([
              (supabase as any).from('loan_notes').insert({
                  loan_id: oldLoan.id,
                  user_id: profile.id,
  ...
  if (loading || !oldLoan) return <div className="p-12 text-center"><RefreshCw className="h-8 w-8 animate-spin mx-auto text-indigo-600" /></div>;
