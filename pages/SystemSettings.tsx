...
export const SystemSettings: React.FC = () => {
  ...
  const handleOptimize = async () => {
      ...
      try {
          const { data, error } = await (supabase as any).rpc('optimize_database');
  ...
  const handleRequestReset = async () => {
      ...
      try {
          ...
          // Use case-insensitive role matching to handle 'CEO', 'ceo', 'ADMIN', 'admin', etc.
          ...
          if (execError) {
              console.error('Failed to fetch executives:', execError);
              toast.error('Could not notify executives. Please inform them manually.');
          } else if (executives && executives.length > 0) {
              const notifications = executives
                .filter((c: any) => c.id !== profile?.id) // Don't notify self
                .map((c: any) => ({
                  user_id: c.id,
              ...
              if (notifications.length > 0) {
                  // Add explicit defaults for all columns to avoid 400 errors
                  const notificationsWithDefaults = notifications.map((n: any) => ({
                      ...n,
                      priority: 'high',
                      category: 'system',
                      is_read: false,
                  ...
                  const { data: notifData, error: notifError } = await (supabase as any)
                      ...
                      if (notifError) {
                          toast.error('Reset logged but notification failed: ' + notifError.message);
                      }
              }
          } else {
              console.warn('No executives found in database to notify');
              toast.error('Request logged but no other executives found to notify.');
          }

          setShowWipeModal(false);
          setConfirmText('');
          fetchPendingReset();
          toast.success('Reset request submitted. Waiting for second authorization.');
      } catch (e: any) {
  ...
  const handleCancelRequest = async () => {
      ...
      if (!window.confirm('Are you sure you want to cancel the factory reset request? This action cannot be undone.')) return;

      setIsProcessing(true);
      try {
          // Update the audit log to show cancellation
          ...
          setPendingRequest(null);
          toast.success('Reset request cancelled successfully.');
          
          // Optionally notify other admins that the request was cancelled
          ...
          if (otherAdmins && otherAdmins.length > 0) {
              const notifications = otherAdmins.map((admin: any) => ({
                  user_id: admin.id,
                  title: 'Factory Reset Request Cancelled',
                  message: `${profile.full_name} has cancelled the factory reset request.`,
                  type: 'info',
                  priority: 'medium',
                  category: 'system',
                  is_read: false,
